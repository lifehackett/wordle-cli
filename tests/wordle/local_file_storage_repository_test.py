...
def test_missing_datastore_file():
    try:
        os.remove(os.getenv("DATASTORE_PATH"))
...
class TestGet:
    def test_missing_key_success(self, repository: IRepository):
        value = repository.get("unknown_key")
        assert value is None


class TestSave:
    def test_new_key_success(self, repository: IRepository):
        repository.save("test_key", "test_value")
        value = repository.get("test_key")
        assert value == "test_value"

    def test_existing_key_success(self, repository: IRepository):
        repository.save("test_key", "test_value")
        repository.save("test_key", "updated_test_value")
        value = repository.get("test_key")
        assert value == "updated_test_value"

    def test_save_dict_success(self, repository: IRepository):
        repository.save("test_key", {"foo": "bar"})
        value = repository.get("test_key")
        assert value == {"foo": "bar"}

    def test_save_list_success(self, repository: IRepository):
        repository.save("test_key", ["foo", "bar"])
        value = repository.get("test_key")
        assert value == ["foo", "bar"]
